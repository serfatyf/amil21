...
// fin de l'inutile (?)
	...
	if ($type != "1") {		// si ce n'est pas "toutes", c'est qu'on a choisi un type d'activites
		$sql .= " WHERE id_typeact = ? ";		// on met donc une condition a la requete
...
echo "act<br>";var_dump($_GET['type']);
echo "bind<br>";	var_dump(($bind));
echo "public<br>";	var_dump($_GET['public']);
echo "sql<br>";	var_dump($sql);
...
<div>
	...
	<button id="1" <?php if ($type=='1') echo " class='checked'" ?> >Tous types</button>
<?php 
